...
@interface MyPhamViewController ()
    
@end

@implementation MyPhamViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.urlDictionary = [[NSMutableDictionary alloc] init];
    [self.urlDictionary setObject: [NSString stringWithFormat: @"%@/topics/7/photos?cityId=%ld&t=popular",[AppConfig GetBaseNextPageUrl], [AppConfig GetCityID]] forKey: @" Phổ biến "];
    [self.urlDictionary setObject: [NSString stringWithFormat: @"%@/topics/7/photos?cityId=%ld&t=latest",[AppConfig GetBaseNextPageUrl], [AppConfig GetCityID]] forKey: @" Mới nhất "];
    [self.urlDictionary setObject: [NSString stringWithFormat: @"%@/topics/7/photos?cityId=%ld&t=opening",[AppConfig GetBaseNextPageUrl], [AppConfig GetCityID]] forKey: @" Đang mở cửa "];
    ...
    [self.headerTitleLabel setText:@"Điệu tí nào"];
    ...
    [self.headerDescriptionLabel setText:@"Phấn son tô điểm sơn hà, làm cho rõ mặt đàn bà nước Nam. Đăng bán 👉Bán👉Mỹ phẩm👉Chíu bùm"];
    ...
    [self.foodCollectionView setDataLoader: [[FoodLoader alloc]init] withStartPage:[NSString stringWithFormat: @"%@/topics/7/photos?t=popular", [AppConfig GetBaseNextPageUrl]]];
